...
// ---------------------------------------------------------------------------
// Timing utilities: mode-agnostic. Admission control is observed through
// ingest_external's accepted/rejected return; the eviction sweep is observed
// through its downstream effect on the next arbitrate() output.
// ---------------------------------------------------------------------------
...
TEST(TrafficLightArbiterCoreTiming, ingestExternalAcceptsStampsWithinTolerance)
{
  ...
  const auto accepted = arbiter.ingest_external(
